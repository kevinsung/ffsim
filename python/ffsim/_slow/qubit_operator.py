...
def _validate_term(term: frozenset[QubitAction]) -> None:
    """Raise a ValueError if a term is not a valid Pauli string."""
    qubits = set()
    for pauli, qubit in term:
        if pauli not in ("X", "Y", "Z"):
            raise ValueError(f"Pauli label must be 'X', 'Y', or 'Z'. Got {pauli}.")
        if qubit in qubits:
            raise ValueError(
                "Term contains more than one Pauli operator acting on the same qubit."
            )
        qubits.add(qubit)


def _z_term_trace(norb: int, nelec: int, num_z: int) -> int:
    """Trace of a product of Z operators over one spin sector.

    Returns the sum over all ``nelec``-electron configurations of ``norb`` orbitals of
    the product of Z eigenvalues on the ``num_z`` marked orbitals, where Z gives +1 for
    an unoccupied orbital and -1 for an occupied one.
    """
    if num_z > norb:
        return 0
    remaining = norb - num_z
    return sum(
        (-1) ** k * math.comb(num_z, k) * math.comb(remaining, nelec - k)
        for k in range(min(num_z, nelec) + 1)
        if nelec - k <= remaining
    )
...
class QubitOperator(MutableMapping):
    ...
    def simplify(self, tol: float = 1e-12) -> None:
    ...
    def _trace_(self, norb: int, nelec: tuple[int, int]) -> complex:
        """Return the trace of the operator in the given Fock space sector."""
        n_alpha, n_beta = nelec
        result: complex = 0
        for term, coeff in self.coeffs.items():
            # Any X or Y flips the qubit, so <s|P|s> = 0; skip this term.
            if any(pauli in ("X", "Y") for pauli, _ in term):
                continue
            qubits = [qubit for _, qubit in term]
            # A Z outside the system has zero trace in the full qubit space.
            if any(qubit < 0 or qubit >= 2 * norb for qubit in qubits):
                continue
            n_alpha_z = sum(qubit < norb for qubit in qubits)
            n_beta_z = len(qubits) - n_alpha_z
            result += (
                coeff
                * _z_term_trace(norb, n_alpha, n_alpha_z)
                * _z_term_trace(norb, n_beta, n_beta_z)
            )
        return result
    ...
    def __repr__(self) -> str:
        def _term_repr(term: frozenset[QubitAction]) -> str:
            ...
            # QubitAction is a NamedTuple, so QubitAction(p, q) == (p, q) and they
