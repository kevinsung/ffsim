...
fn extract_term(obj: &Bound<'_, PyAny>) -> PyResult<PauliTerm> {
    ...
    for item in obj.try_iter()? {
        ...
        if !matches!(pauli_byte, b'X' | b'Y' | b'Z') {
            return Err(PyValueError::new_err(
                "Pauli label must be 'X', 'Y', or 'Z'.",
            ));
        }
        actions.push((pauli_byte, qubit));
    }
    actions.sort_unstable();
    // Actions are sorted by (pauli, qubit), so two Paulis on the same qubit need not be
    // adjacent. Check for repeated qubits separately; otherwise one of them would be
    // silently dropped when the term is keyed by qubit during multiplication.
    let mut qubits: Vec<i32> = actions.iter().map(|&(_, qubit)| qubit).collect();
...
impl QubitOperator {
    ...
    fn __mul__(&self, other: &Self) -> Self {
        ...
        for (term1, coeff1) in &self.coeffs {
            for (term2, coeff2) in &other.coeffs {
                ...
                let val = coeffs.entry(new_term).or_default();
                *val += new_coeff;
            }
        }
        Self { coeffs }
    ...
    fn __pow__(&self, exponent: i64, modulo: Option<i64>) -> PyResult<Self> {
        if modulo.is_some() {
            return Err(PyValueError::new_err("mod argument not supported"));
        }
        if exponent < 0 {
