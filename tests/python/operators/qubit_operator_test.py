...
def test_simplify():
    ...
    op = QubitOperator(
        {
            ...
            frozenset({ffsim.z(2)}): 1e-13,
            frozenset({ffsim.x(3)}): -1e-11,
            frozenset({ffsim.y(4), ffsim.z(5)}): 0.5,
            frozenset({ffsim.x(0)}): 1e-14 + 2e-14j,
        }
    )
    ...
    small_op = QubitOperator(
        {
            frozenset({ffsim.x(0), ffsim.y(1)}): 1e-13,
            frozenset({ffsim.z(2)}): 1e-14,
        }
    )
    small_op.simplify()
...
def test_trace():
    ...
    expected: complex = 0.5 * dim + 1.5 * 3 + 2.0 * 0
